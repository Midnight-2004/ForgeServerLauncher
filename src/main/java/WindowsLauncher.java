...
public class ForgeServerLauncher {
    ...
    public static void main(String[] args) {
        ...
        String neoforgePath = "libraries\\net\\neoforged\\neoforge";
        String forgePath = "libraries\\net\\minecraftforge\\forge";
        ...
        // 定位并读取win_args.txt文件
        File winArgsFile = new File(latestVersionDir, "win_args.txt");
        if (!winArgsFile.exists()) {
            System.out.println("win_args.txt not found in the latest version directory.");
            return;
        }

        // 从文件中读取启动参数
        List<String> launchArguments = readArgumentsFromFile(winArgsFile.getAbsolutePath());
        if (launchArguments.isEmpty()) {
            System.out.println("Failed to read arguments from win_args.txt.");
            return;
        }
